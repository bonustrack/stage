/** @file XMTP send helpers (text, reactions, votes, replies, attachments, signatures, transactions) for the app's XMTP client lib; extracted from lib/xmtp.ts, with the decoded-message-to-envelope mapper re-exported from xmtp.envelope.ts. */
...
/** Send an XMTP reaction (action=added) or removal (action=removed) targeting an existing message id in the same conversation. */
...
/** Send a signature REQUEST (`metro.box/signatureRequest:1.0`) — either EIP-712 typed data or a personal_sign string. Mirrors xmtpSendPoll. */
...
/** Post a payment RECEIPT (`xmtp.org/transactionReference:1.0`) back into the conversation after the payer broadcasts the tx. Mirrors xmtpSendPoll. */
...
/** Submit (or retract) a FREE-TEXT answer to an open poll question. Rides the vote pipeline: a custom-schema reaction whose content is `open:<q>:<base64>`. An empty `text` retracts the voter's prior answer. */
...
/** Send an inline (static) XMTP attachment. `dataB64` is the raw bytes base64-encoded (matches the RN SDK bridge convention). Good for files < ~1 MB; larger payloads should use the remote-attachment flow (TODO). */
