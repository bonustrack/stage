 ...
 *  cached value without touching storage. Missing → OFF (default). */
...
/** Synchronous read from the in-memory cache. Returns the default (false) until
 *  `loadDebugConsole` has run. */
...
/** React hook: reactive read of the gate. Kicks off the one-time load on mount
 *  so a fresh process reflects the persisted value, then re-renders on flips. */
