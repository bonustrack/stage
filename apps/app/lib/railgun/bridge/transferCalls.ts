/** RN-side typed bridge wrappers for the RAILGUN private-TRANSFER primitives.
 *
 ...
 *
 *  All three are whitelisted in sdkDispatch.js. Transfer's SDK signatures carry
 *  two args unshield lacks — `showSenderAddressToRecipient` and `memoText` — we
 *  fix both (don't reveal sender, no memo). bigint amounts are wire-encoded
 *  ({ __bigint }) so they survive the JSON channel and are revived in the host.
 *  Self-broadcast only: sendWithPublicWallet=true, no broadcaster fee; the EOA
 *  signs + broadcasts the returned tx on RN. */
import { sdk } from './sdk';
import { bn } from './wire';
...
export async function gasEstimateTransfer(params: {
  txidVersion: string;
  networkName: string;
  ...
  return sdk<{ gasEstimate: string }>('gas.estimateTransfer', [
    params.txidVersion,
    params.networkName,
...
export async function populateProvedTransfer(params: {
  txidVersion: string;
  networkName: string;
  railgunWalletID: string;
  erc20Recipients: TransferErc20Recipient[];
  ...
  return sdk<PopulateResult>('tx.populateProvedTransfer', [
    params.txidVersion,
    params.networkName,
    params.railgunWalletID,
    false, // showSenderAddressToRecipient
