/** Pure asset-resolution helpers for the tx/sign cards + simulation.
 *
 *  Maps a `(chainId, tokenAddress|null)` pair to:
 *    - its DISPLAY LOGO url (the real token icon, never a wrong fallback), and
 *    - its CoinGecko PRICE KEY (native coin id, or contract+platform), so the
 *      price layer (lib/txPrices) can quote a USD value next to the amount.
 *
 *  Both are derived from the shared static registry (packages/client assets) so
 *  the cards show the SAME logo + price source as the wallet token list. No
 *  React, no network, no key material — bytes in, descriptors out (unit-testable
 *  without pulling in RN). */
...
/** Whether the unknown-token logo should defer to a neutral in-app glyph rather
 *  than a stamp.fyi identicon (caller decides). True only for unknown ERC-20s. */
 ...
 *  known price source (registry has no cgId/cgPlatform — show amount only). */
