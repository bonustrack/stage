...
export function diffMarkdownStyles(p: DiffMdPalette, dark: boolean): Record<string, object> {
  ...
  return {
    ...
    strong: { fontFamily: 'Calibre-Semibold', fontWeight: 'normal', fontSize: 17, lineHeight: lh },
    em: { fontFamily: 'Calibre-Medium', fontStyle: 'italic', fontWeight: 'normal', fontSize: 17, lineHeight: lh },
    link: { color: p.link, textDecorationLine: 'underline' },
    /** Menlo's em-square is wider than Calibre's, so size down to match. */
    code_inline: { backgroundColor: codeBg, paddingHorizontal: 4, borderRadius: 4, fontFamily: 'Menlo', fontSize: 14, lineHeight: lh },
