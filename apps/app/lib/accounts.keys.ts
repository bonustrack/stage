...
/** Read the raw private key for an account id. This is an EXPORT/REVEAL path —
 *  callers (the "Export private key" UI) gate it behind a destructive warning
 *  Alert. Delegates to the keyring's single reveal accessor. */
export const getPrivateKey = revealPrivateKey;

/** PURE rules only (no storage / no secrets). The PK storage-key CONSTANTS
