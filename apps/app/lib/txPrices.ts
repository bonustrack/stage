/** USD-price layer for the tx/sign cards + simulation asset lines.
 *
 *  Given a `(chainId, token)` the registry knows how to price (lib/txAssets),
 *  fetch the spot USD price from CoinGecko (the SAME source + key the wallet tab
 *  uses), cache it briefly, and expose a hook that turns a token amount into a
 *  `~$X` suffix. Non-blocking: the card renders the amount immediately and the
 *  `~$X` appears once the (cached) price resolves. Tokens with NO known price
 *  source (e.g. STAGE) resolve to `null` and show the amount ONLY — never a fake
 *  or zero $.
 *
 *  READ-ONLY price fetch, no key material (no-keyring-bypass). */
...
/** In-memory price cache keyed by priceKeyId; entries expire after TTL so a long
 *  session re-quotes occasionally without hammering CoinGecko. */
const TTL_MS = 60_000;
...
/** Format `amount × priceUsd` as a `~$X` suffix string, or null when there's no
 *  price. Sub-cent values still render (`~$0.00…` is suppressed -> show < $0.01
 *  as "~$0.01" floor so it never reads as free). */
...
/** React hook: resolve the `~$X` suffix for a single `(chainId, token, amount)`.
 *  Non-blocking — returns null until the price resolves; null forever for an
 *  unpriceable token. */
