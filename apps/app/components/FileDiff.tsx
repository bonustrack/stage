  ...
  return (
    <Box>
      <ListViewItem dark={dark} gap={8} onPress={() => setOpen(o => !o)}>
        ...
        <Text size="sm" numberOfLines={1} style={{ flex: 1, color: p.text, fontFamily: FONT }}>
          {file.filename}
        </Text>
        <Text size="sm" style={{ color: p.success, fontFamily: FONT }}>+{file.additions}</Text>
        <Text size="sm" style={{ color: p.danger, fontFamily: FONT }}>-{file.deletions}</Text>
      </ListViewItem>
      {open ? (
        file.noPatch ? (
          <Box style={{ paddingHorizontal: 16, paddingBottom: 10 }}>
            <Text size="sm" style={{ color: p.text, opacity: 0.6, fontFamily: FONT }}>
                ...
                <Box key={i} style={{ flexDirection: 'row', backgroundColor: lineBg(ln.kind, dark), minWidth: '100%' }}>
                  {ln.kind === 'hunk' ? null : (
                    <Box style={{ flexDirection: 'row', borderRightWidth: 1, borderRightColor: p.border }}>
                      <Text size="xs" style={{ width: 36, textAlign: 'right', color: p.text, opacity: 0.4, fontFamily: FONT, lineHeight: 18, paddingRight: 4 }}>
                        {gutter(ln.oldLine)}
                      </Text>
                      <Text size="xs" style={{ width: 36, textAlign: 'right', color: p.text, opacity: 0.4, fontFamily: FONT, lineHeight: 18, paddingRight: 4 }}>
                        {gutter(ln.newLine)}
                  ...
                  <Text size="sm" style={{ width: 12, marginLeft: 6, color: lineColor(ln.kind, p), fontFamily: FONT, lineHeight: 18 }}>
                    {marker(ln.kind)}
                  </Text>
                  <Text size="sm" style={{ color: lineColor(ln.kind, p), fontFamily: FONT, lineHeight: 18, paddingRight: 12 }}>
                    {ln.text || ' '}
                  </Text>
                </Box>
              ))}
