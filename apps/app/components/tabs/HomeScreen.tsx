...
export function HomeScreen({ panRef }: { panRef?: SimultaneousRefs } = {}): React.ReactElement {
  ...
  /** Wrap setRows so every state update also lands in the shared cache + fans out to subscribers (e.g. the conv view's markConvRead). */
  ...
  /** Mirror the channels cache into TanStack Query (stage-1 cache unification) so read-only consumers dedupe off one entry. The cache stays the writer; this screen's own state path above is unchanged. */
  ...
  /** Channels-list scroll persistence: ref, saved offset, one-shot restore flag, measured content height (clamp the saved offset to it). */
  ...
  /** Filter chip state: enabled label set (OR-filter) + the built-in "Unread" toggle, with the "All" clear-all handler. unreadOnly AND-narrows to conversations with unread messages (unreadCount > 0 OR markedUnread). */
  ...
  useEffect(() => {
    ...
    /** On toggle the cache is already updated; re-read it (resolves instantly once loaded) into a fresh Set so React sees a new reference. */
  ...
  const sortedRows = useMemo(() => {
    /** Archived convs are removed from the main list entirely (shown only in the Archived view). Filtered first so the label filter + sort operate on the visible set. */
    const all = (rows ?? []).filter(r => !archived.has(r.convId));
    /** Label filter (ANY/OR): no enabled labels → show all; otherwise keep channels carrying at least one enabled label (case-insensitive). Reactive to `rows` so cache updates re-derive. */
  ...
  /** Search query applied on top of the sorted/archived/label-filtered list (so search never surfaces hidden channels). Empty query = full list. */
  ...
  /** Stable extraData array (identity only changes when a version does) so the FlatList doesn't re-render the whole window each stream tick. */
