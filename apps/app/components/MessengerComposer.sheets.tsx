  ...
  return (
    <AppModal visible={open} onClose={onClose}>
      <Col gap={12} pb={8}>
        ...
        >
          <Text style={{ color: fg, fontSize: fontSize('md'), fontFamily: 'Calibre-Medium' }}>Allow multiple choices</Text>
          <Box style={{
      ...
      <Col gap={12} pb={8}>
        <Row gap={8}>
          {([['personal', 'Message'], ['eip712', 'Typed data']] as const).map(([k, label]) => (
            ...
            >
              <Text style={{ color: kind === k ? '#c0a06e' : fg, fontSize: fontSize('md'), fontFamily: 'Calibre-Semibold' }}>{label}</Text>
            </Pressable>
          ))}
        </Row>
        <Input
        ...
        {kind === 'personal' ? (
          ...
          <Textarea
            ...
            style={{ color: fg, backgroundColor: inputBg, borderRadius: r, paddingHorizontal: 12, paddingVertical: 10, minHeight: 160, height: undefined, fontFamily: 'Calibre-Medium', fontSize: fontSize('sm'), textAlignVertical: 'top' }}
          />
        )}
        <Button
          variant="primary"
