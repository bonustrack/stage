...
function PollQuestionBlock({ q, qi, sub, dark, votes, own, onVote, openAnswers, mine, onOpenAnswer }: {
  ...
  return (
    <Box style={{ alignSelf: 'stretch', gap: 6 }}>
      {q.header ? (
        <Text size="sm" style={{ color: sub, fontFamily: 'Calibre-Semibold', textTransform: 'uppercase', letterSpacing: 0.5 }}>
        ...
        return (
          ...
          >
            ...
            <Row align="center" justify="between">
              <Text size="lg" style={{ color: isOn ? '#fff' : pal.text, fontFamily: 'Calibre-Medium', flexShrink: 1 }}>
                {isOn ? '✓  ' : (multi ? '☐  ' : '')}{opt.label}
              </Text>
              <Text size="md" style={{ color: isOn ? '#fff' : sub, fontFamily: 'Calibre-Semibold', marginLeft: 8 }}>
                {count}
              </Text>
            </Row>
            {opt.description ? (
    ...
    <Box style={{ alignSelf: 'stretch', gap: 12, marginTop: 8 }}>
      {poll.questions.map((q, qi) => (
        <Box key={`q-${qi}`} style={{ alignSelf: 'stretch', gap: 6 }}>
          {multiQuestion && qi > 0 ? (
            <Text size="xl" style={{ color: fg, fontFamily: 'Calibre-Semibold' }}>{q.question}</Text>
          ) : null}
          <PollQuestionBlock
