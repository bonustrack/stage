    ...
    <Box style={{ paddingHorizontal: 18, paddingBottom: 16 }}>
      {rec ? (
        <>
          ...
          <Text size="xl" numberOfLines={1} style={{ color: c.head, fontFamily: 'Calibre-Semibold' }}>
            {getPeerName(rec.address) ?? rec.label ?? shortAddress(rec.address)}
          </Text>
          <Text size="md" numberOfLines={1} style={{ color: c.sub, fontFamily: 'Calibre-Medium', marginTop: 1 }}>
  ...
  return accounts.map((a) => (
    <ListViewItem key={a.id} dark={dark} onPress={() => onSwitch(a.id)}>
      ...
      <Box style={{ flex: 1, minWidth: 0 }}>
        <Text size="md" numberOfLines={1} style={{ color: c.head, fontFamily: 'Calibre-Semibold' }}>
          {getPeerName(a.address) ?? a.label ?? shortAddress(a.address)}
        </Text>
        <Text size="sm" numberOfLines={1} style={{ color: c.sub, fontFamily: 'Calibre-Medium', marginTop: 1 }}>
...
export function DrawerRow({ rowKey, icon, label, onPress, head, sub, dark }: {
  ...
  return (
    <ListViewItem key={rowKey} dark={dark} onPress={onPress}>
      ...
      <Box style={{ flex: 1 }}>
        <Text size="lg" style={{ color: head, fontFamily: 'Calibre-Medium' }}>{label}</Text>
      </Box>
