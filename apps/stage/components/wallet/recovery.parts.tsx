...
export function GuardianEditor({
  ...
  return (
    <Col gap={16}>
      ...
      {n > 0 ? (
        <Col gap={8}>
          <Text size="xs" role="secondary">GUARDIANS ({n})</Text>
          {guardians.map(addr => (
      ...
      <Text size="xs" role="secondary">
        A recovery takes effect only after a {formatDelay(delaySeconds)} delay.{' '}
        <Text size="xs" role="link">Funds can never move during recovery.</Text>
      </Text>
    </Col>
  );
}
