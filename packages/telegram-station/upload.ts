/** Telegram outbound dispatch: text + photos + documents + voice + buttons. */
...
export type RichOpts = { images?: string[]; documents?: string[]; voice?: string; buttons?: Button[][] };

export const inlineKeyboard = (rows: Button[][]): { inline_keyboard: Button[][] } => ({ inline_keyboard: rows });
...
/** Dispatch text/images/documents/voice/buttons; returns the first message id. */
export async function tgSendRich(
  token: string, send: TgSendApi, base: Record<string, unknown>, text: string, opts?: RichOpts,
): Promise<string> {
