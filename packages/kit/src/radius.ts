/** ChatKit Box/Row/Col `radius` token enum + scale (BlockProps.radius).
 *
 *  Split out of tokens.ts to keep that file under the 400-line cap. Re-exported
 *  from ./tokens (and so from the package index) so existing import paths are
 *  unaffected. This is the per-block corner-radius scale; the theme-level
 *  `RadiusName` (pill/round/soft/sharp) in tokens.ts is a separate concept. */
