...
import { HERO_ICON_PATHS, HERO_SOLID_ICON_PATHS, type HeroIconName } from '../src/icons';
...
const SOLID_NAMES = HERO_NAMES.filter((name) => name in HERO_SOLID_ICON_PATHS);
...
Controls.args = { name: 'chatBubble', size: 32, focused: false, variant: 'outline' };
Controls.argTypes = { name: select(HERO_NAMES), size: range(12, 96, 4), color, focused: bool, variant: select(['outline', 'solid']) };
  ...
  return (
    <Row gap={12} wrap>
      {SOLID_NAMES.map((name) => (
        <Col key={name} width={96} align="center" gap={4} padding={{ y: 8 }}>
          <Icon name={name} variant="solid" size={24} dark={dark} />
          <Text size="3xs" role="secondary" truncate>{name}</Text>
        </Col>
      ))}
    </Row>
