 ...
 *  point `rpcUrls.default` at flaky public endpoints, so always route reads
 *  through this instead of a bare `http()`. */
export function broviderTransport(chainId: number) {
  return http(broviderRpc(chainId));
}

/** A viem chain for ANY chainId. Known chains come from VIEM_CHAINS; unknown
 ...
 *  generic chain definition for chainIds not in VIEM_CHAINS. */
