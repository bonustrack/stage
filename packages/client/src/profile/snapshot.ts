/** Stamp.fyi identity helpers shared by web (apps/ui) and mobile (apps/app).
 *  Identity is READ-ONLY and resolved entirely from stamp.fyi / ENS — there is
 *  no in-app profile editing and no Snapshot hub usage. Keep dependency-free. */

/** stamp.fyi JSON-RPC endpoint. Snapshot's own UI resolves display names from
 *  here (`lookup_addresses` → ENS) and serves identicon/ENS avatars. */
