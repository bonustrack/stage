...
/** The minimal cache surface the resolution rule needs. The SDK's default
 *  `InboxEthCache` implements it; the RN app can also pass its existing
 *  pub/sub-backed MemoryStore adapter so the SDK rule drives ONE cache instead of
...
/** A tiny in-memory inbox-id -> ETH-address cache. Distinct class (rather than a
 *  bare Map) so the resolution rule has one place to clear on account switch and
 *  the host can inject a richer store later if needed. */
...
/** A fetcher that resolves a batch of inbox ids to ETH addresses over the
 *  network. The host implements it on top of the native client's
 *  `inboxStates(true, ids)`; missing ids may be absent from the returned map. */
