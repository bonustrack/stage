...
/** One question of a (possibly multi-question) poll. Mirrors a single entry in
 *  AskUserQuestion's `questions[]`. The QUESTION INDEX (position in `questions[]`)
 *  plus the OPTION INDEX form the canonical vote key. */
...
export interface PollContent {
  /** Stable id minted at creation; survives edits/resends so votes can
   *  reference it. (The canonical vote reference is still the message id.) */
  pollId: string;
  /** AskUserQuestion-shaped MULTI-question form: one block per question. When
   *  present (non-empty) this is authoritative; the legacy top-level fields are
   *  ignored. New senders should prefer this. */
  questions?: PollQuestion[];
  /** LEGACY single-question prompt. Kept for backward compat with v1 polls;
   *  `normalizeQuestions()` folds either shape into one `PollQuestion[]`. */
  question?: string;
...
/** Fold either poll shape into a `PollQuestion[]`. Multi-question polls return
 *  their `questions[]`; legacy single-question polls return a one-element array.
...
/** Mint a stable poll id. Uses `crypto.randomUUID()` where available (RN
 *  Hermes + modern browsers via the app's existing getRandomValues polyfill),
 *  with a cheap fallback so the helper never throws in a bare environment. */
 ...
 *  clients render something readable) and as the envelope `text`. */
