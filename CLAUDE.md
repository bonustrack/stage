...
| `apps/proxy` | — | Cloudflare Worker on proxy.stage.box: link previews, `/img` resize, x402 challenge + settle, the `/names/*` stage-names service (operator key, claims serialized in the `NamesClaims` Durable Object with KV as the read mirror, server-side label validation), `/xmtp-history/*` and `/xmtp-push/*` relays, plus the bundler.stage.box per-branch manifest proxy. Deployed by `deploy-proxy.yml`. |
