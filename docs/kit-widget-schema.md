...
- **Modal / overlay shells** — these now render through the `Dialog` node (Section 4): `AppModal.tsx` (and its consumers `MenuSheet.tsx`, `ChannelMenu.tsx`), `AccountsManager` `SheetModal`, `ImageViewer.tsx`, the xmtp bubble action menu (`xmtp-conv/parts.tsx`), and the Vue `*ComposeSheet.vue` (`PollComposeSheet`, `PaymentComposeSheet`, `SignRequestComposeSheet`) all build a `Dialog` and render its panel content as JSON/kit leaves. The `Dialog` node owns the `Modal`/fixed-overlay shell (backdrop, animation, safe-area, keyboard, dismiss); these files are now thin containers that own open-state + content, not bespoke overlays. Remaining raw-`Modal`/fixed-overlay code outside the `Dialog` node is limited to the wallet `TokenSelector`/sheet bodies (not yet migrated) and continuous-gesture sheets (next item).
