...
- `animationType`: `"slide" | "fade" (default) | "none"` — RN `Modal.animationType`; Vue maps to an equivalent CSS transition (`slide` = panel `translateY(100%)` + fade, `fade` = opacity, `none` = no transition). `side: "bottom"` implies a slide transition on Vue.
- `backdropColor`: `Color` (default `rgba(0,0,0,0.5)`) — scrim color.
- `gestureRoot`: bool (default `false`) — wrap the overlay in a `GestureHandlerRootView` (RN). Needed when the panel hosts gesture-handler children inside the `Modal` portal.
- `safeAreaBottom`: bool (default `false`) — add `useSafeAreaInsets().bottom` to the panel's bottom padding (RN).
- `panelBackground`: `Color`, `panelRadius`: `RadiusValue | number` (top corners only when `side: "bottom"`, all four when centered), `panelMaxHeight`: `Dimension`, `panelPadding`: `SpacingValue`, `panelBorderColor`: `Color` (1px top border) — panel surface styling.
- `handle`: bool — render a centered 36×4 grab-handle bar at the top of the panel; `handleColor`: `Color` (default `rgba(0,0,0,0.2)`).
- `scroll`: bool — wrap the panel content in a vertical `ScrollView`; `keyboardPersistTaps`: bool (default `true` when `scroll`) → `keyboardShouldPersistTaps="handled"` (`"never"` when `false`); `scrollPadding`: `SpacingValue` → the scroll `contentContainerStyle` padding.
- `fullBleedPanel`: bool — the panel fills the whole overlay (`flex: 1`, `pointerEvents="box-none"`) instead of a sized sheet, for full-screen overlays (image viewer) and absolutely-positioned content (bubble action menu) whose children own their own positioning and presses.

RN renders a transparent `Modal` (`animationType` per field, `onRequestClose` → close) with an overlay `Pressable` (`justifyContent`/`alignItems` by `side`/`fullBleedPanel`, scrim `backgroundColor` when `backdrop`), and the panel `Pressable` above it (`stopPropagation` so panel taps don't dismiss; sized + styled by the panel fields, optionally scroll-wrapped); Vue renders a `fixed inset-0 z-50` flex overlay with a `Transition` (name per `animationType`/`side`), an `absolute inset-0` outside-click backdrop (`backdropColor`), and a `position:relative` panel holding the children (node mode) or a default `<slot>` (component mode). The app's modal/overlay shells (`AppModal`, `MenuSheet`/`ChannelMenu` via `AppModal`, `AccountsManager` `SheetModal`, `ImageViewer`, the xmtp bubble action menu, and the Vue `*ComposeSheet.vue`) render through this node via the `@stage-labs/kit/react-native/dialog` `Dialog` component (RN) and the `<Dialog>` Vue component (slot mode).
...
This section records the rendering boundary established by the JSON-renderer refactor (PR #636).

**Every visual component and leaf renders from Kit JSON.** Across both clients, visual trees are authored as the OpenAI-compatible widget JSON documented above, loaded by `buildView` from `.json` view files in `@stage-labs/views` (53 JSON views, paired 1:1 with their `.ts` loaders), and rendered by a single `KitRenderer` (parallel React Native + Vue implementations). 93 components across `apps/app` (48) and `apps/ui` (45) render their visual content through `KitRenderer`. Interactivity flows through the shared `WidgetActionRegistry`, so the JSON `{type, payload}` actions resolve to native handlers on both platforms.

**The remaining non-JSON code is a small, exhaustively enumerated set of structural and framework shells** — never a visual component or leaf. Every shell either mounts or animates around JSON-rendered content; none of them is a UI component you would see as a pixel of product surface. Per the repo `CLAUDE.md` native-exception convention (the kit-only rules sanction `<component :is="…">` with a `kit-exception` comment in Vue, and kit RN primitives + a thin layout layer in React Native), the complete list is:

- **(a) Framework route / layout / provider roots** — `apps/app/app/_layout.tsx` and the expo-router / vue-router route + layout files. They wire providers (theme, query, gesture, keyboard) and the navigation stack; they own no pixels, they mount the trees that do. (Modal / overlay surfaces are no longer in this set — they now render through the `Dialog` node, Section 4: `AppModal.tsx` and its consumers, `AccountsManager` sheets, `ImageViewer.tsx`, the xmtp bubble action menu, and the Vue `*ComposeSheet.vue` all build a `Dialog` and render its panel content as Kit JSON.)
- **(b) Continuous-animation drivers** — `Spinner.tsx`, `SwipeTabs.tsx`, `PullToRefresh.tsx`, `MessengerBubble.tsx` (highlight background), `WalletScreen.refreshButton.tsx`, and the conversation screen's reanimated scroll. Per-frame `Animated` / reanimated shared values (voice waveform, highlight fade, pull/refresh, swipe) cannot be expressed as a static JSON tree; the driver animates *around* JSON leaves — its rendered content is JSON.
- **(c) Two structural containers (Vue) — the only remaining `kit-exception` sites in `apps/ui`, exactly two:**
  - `XmtpConversation.vue` — the ref-measured **scroll-measurement viewport**: a composable reads `scrollTop`/`scrollHeight` and needs `absolute inset-0` + `no-scrollbar`, which the Kit `Scroll` node cannot express.
  - `MessengerBubble.vue` — the **inline-flow paragraph container**: a normal block (not a Kit `Col`, which forces `display:flex/column` and would stack segments vertically) so the mention link + surrounding text read as one paragraph, matching mobile's `MentionBody` single `Text` run.

  Each carries an inline `<!-- kit-exception: … -->` justification, and neither can become JSON without breaking exact UI/UX. They are structural containers, not visual leaves.
